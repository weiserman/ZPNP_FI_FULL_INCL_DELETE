...
define view entity ZAP_I_CHANNEL_VH
  as select from    DDCDS_CUSTOMER_DOMAIN_VALUE(
                      p_domain_name : 'ZAP_DM_CHANNEL') as Values
    left outer join DDCDS_CUSTOMER_DOMAIN_VALUE_T(
                      p_domain_name : 'ZAP_DM_CHANNEL') as Texts on  Texts.domain_name    = Values.domain_name
                                                                ...
                                                                and Texts.language       = $session.system_language
{
  ...
  key Values.value_low as Channel,
      @Semantics.text: true
      @Search.defaultSearchElement: true
